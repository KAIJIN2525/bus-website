...
-import { FaBars, FaPhone, FaUser, FaHeadset } from 'react-icons/fa6';
+import { FaTimes, FaBars, FaPhone, FaUser, FaHeadset, FaSignOutAlt, FaUserCircle } from 'react-icons/fa';
...
+            <Link to={"/"} className='flex items-center flex-shrink-0'>
                 <img src={Logo} alt="logo" className="w-36 h-auto object-contain hover:scale-105 transition-transform duration-200" />
             </Link>
...
+            <button onClick={handleClick} className="lg:hidden text-neutral-600 dark:text-neutral-300 hover:text-violet-600 dark:hover:text-violet-400 ease-in-out duration-300 text-xl">
                 {
                     open ?
...
+            <div className={`${open ? 'flex absolute top-20 left-0 w-full' : 'hidden'} lg:flex flex-col lg:flex-row gap-x-5 gap-y-2 lg:items-center lg:p-0 p-4 justify-between lg:bg-transparent bg-white/95 dark:bg-neutral-900/95 lg:shadow-none shadow-lg backdrop-blur-md lg:rounded-none rounded-md lg:border-none border border-neutral-200/50 dark:border-neutral-800/50 flex-1 lg:mx-16`}>
...
+                    <div className="relative bg-gradient-to-r from-violet-600 to-violet-700 hover:from-violet-700 hover:to-violet-800 rounded-xl px-6 py-3 cursor-pointer transition-all duration-300 shadow-md hover:shadow-lg hidden md:block">
                         ...
                         <div className="space-y-0.5">
                             <p className="text-xs text-violet-100 font-light">
                                 24/7 Support
                             </p>
-                            <p className="text-xs font-semibold text-white tracking-wide">+1 (555) 123-4567</p>
+                            <p className="text-xs font-semibold text-white tracking-wide">+234 800 123 4567</p>
                         </div>
                     </div>
