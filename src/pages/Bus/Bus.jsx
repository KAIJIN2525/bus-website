...
import { FaSearch, FaFilter, FaStar, FaWifi, FaSnowflake, FaUsers } from "react-icons/fa";
...
const Bus = () => {
  ...
  const getCategoryIcon = (category) => {
    switch (category.toLowerCase()) {
      case 'luxury': return FaSnowflake;
      case 'deluxe': return FaWifi;
  ...
  const getCategoryColor = (category) => {
    switch (category.toLowerCase()) {
      case 'luxury': return 'bg-purple-100 text-purple-700 dark:bg-purple-900/20 dark:text-purple-400';
      case 'deluxe': return 'bg-blue-100 text-blue-700 dark:bg-blue-900/20 dark:text-blue-400';
      case 'tourist': return 'bg-green-100 text-green-700 dark:bg-green-900/20 dark:text-green-400';
    ...
    <div className="w-full lg:px-28 md:px-16 sm:px-7 px-4 mt-24 mb-16">
      ...
      <div className="text-center mb-12">
        ...
        <p className="text-lg text-neutral-600 dark:text-neutral-400 max-w-2xl mx-auto">
          Choose from our fleet of comfortable, safe, and reliable buses for your next journey
        </p>
      </div>
      ...
      <div className="w-full bg-white dark:bg-neutral-900/60 rounded-2xl px-6 md:px-8 py-8 mb-12 shadow-xl border border-neutral-200/50 dark:border-neutral-800/50">
        ...
        <div className="mt-6 pt-6 border-t border-neutral-200 dark:border-neutral-700">
          <p className="text-neutral-600 dark:text-neutral-400">
            Showing <span className="font-semibold text-violet-600 dark:text-violet-400">{filteredBuses.length}</span> buses
              ...
              >
                <div className="relative overflow-hidden">
                  ...
                  <div className="absolute top-4 right-4">
                    <span className={`inline-flex items-center gap-1 px-3 py-1 rounded-full text-xs font-semibold ${getCategoryColor(bus.category)}`}>
                    ...
                    <div className="flex items-center justify-between mb-3">
                      <div className="flex items-center gap-1">
                        {Array.from({ length: 5 }, (_, i) => (
                          <FaStar 
                            key={i} 
                            className={`text-sm ${i < Math.floor(bus.rating) ? 'text-yellow-400' : 'text-gray-300 dark:text-gray-600'}`} 
                          />
                        ))}
                      ...
                      <div className="flex items-center gap-1 text-neutral-500 dark:text-neutral-400">
                        ...
                        <span className="text-sm font-medium">{bus.passengers} seats</span>
                      </div>
                    </div>
                  </div>
